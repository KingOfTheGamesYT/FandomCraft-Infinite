...
public class TravellerShopOverworld extends GuiContainer
{
    private static final ResourceLocation texture;
    
    public TravellerShopOverworld(final Container container) {
        super(container);
    ...
    protected void drawGuiContainerBackgroundLayer(final float f, final int x, final int y) {
        ...
        this.mc.renderEngine.bindTexture(TravellerShopOverworld.texture);
        final int xSize = 196;
        final int ySize = 128;
    ...
    protected void drawGuiContainerForegroundLayer(final int par1, final int par2) {
        final int posX = this.width / 2;
        final int posY = this.height / 2;
        this.fontRendererObj.drawString("Halo of the Sun: $1500", posX - 85, posY - 59, 0);
        this.fontRendererObj.drawString("Throwing Knife: $50", posX - 85, posY - 31, 0);
        this.fontRendererObj.drawString("Mining Helmet: $750", posX - 85, posY - 3, 0);
        this.fontRendererObj.drawString("Backpack: $750", posX - 85, posY + 25, 0);
    }
    ...
    protected void actionPerformed(final GuiButton button) {
        ...
        if (button.id == 0) {
            ...
            final APacketPurchase.PurchaseMessage purchaseMessage = new APacketPurchase.PurchaseMessage(player, 1500, HorrorReg.haloOfTheSun);
        }
        if (button.id == 1) {
            Tools.printcon("Sending packet for purchase #2");
            final APacketPurchase.PurchaseMessage purchaseMessage2 = new APacketPurchase.PurchaseMessage(player, 50, HorrorReg.throwingKnife);
        }
        if (button.id == 2) {
            Tools.printcon("Sending packet for purchase #3");
            final APacketPurchase.PurchaseMessage purchaseMessage3 = new APacketPurchase.PurchaseMessage(player, 750, Main.miningHelmet);
        }
        if (button.id == 3) {
            Tools.printcon("Sending packet for purchase #4");
            final APacketPurchase.PurchaseMessage purchaseMessage4 = new APacketPurchase.PurchaseMessage(player, 750, HorrorReg.backpack);
        }
    }
